...
$("#dirselect").ready(function () {
    let dir = {
        depid : $("#dirselect").text(),
        dep : ""
    };
    $.ajax({
        type: "POST",
        ...
        success: function (deps) {
            $depselect.empty();
            $svcselect.empty();
            $depselect.append(`<option>Sélectionnez le Département</option>`);
            $svcselect.append(`<option>...</option>`);
            $.each(deps, function (i, dep) { 
                 $depselect.append(`<option value="${dep.code_dep}">${dep.intitule_dep}</option>`);
            });
        }
    });
    let emp = {
